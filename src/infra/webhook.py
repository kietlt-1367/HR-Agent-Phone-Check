...
class WebhookResult:
    ...
    def __init__(self, success: bool, status_code: Optional[int] = None, error: Optional[str] = None):
...
async def send_interview_webhook(
    ...
    for attempt in range(retry_count):
        try:
            logger.info(f"Sending webhook (attempt {attempt + 1}/{retry_count}): {webhook_url}")
...
def prepare_interview_payload(session_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload["candidate"] = {
        ...
        "applied_position": session_data.get("personal_info", {}).get("applied_position", ""),
    }
    ...
    payload["questions"] = session_data.get("closing_notes", {}).get("candidate_questions", [])
